...
(function() {
    ...
    appendChild(st);window.gegeRenderedMacs=new Set();async function rSD(pWT=null,sT=null){if(
        ...
        parseFloat(d.UpThroughput)||0)*8000,dT=(parseFloat(d.DownThroughput)||0)*8000;cI[m]={upRate:u,dnRate:dn,iface:
                                                                                             d.Interface||"",offUp:uT,offDn:dT,onSec:parseInt(d.OnlineDuration||d.OnlineTime||d.LeaseTime||0)};cSU+=u;cSD+=dn;}});let ol=document.getElementById('gege-global-overlay'),cM=Object.keys(
    ...
    lT){aT.classList.add('router-link-exact-active','router-link-active');lT.classList.add('is-active');lT.style.color=
        'rgb(61, 163, 247)';}if(!o){o=document.createElement('div');o.id='gege-global-overlay';let pT=document.querySelector('.page-top');requestAnimationFrame(()=>{if(pT){pT.parentNode.style.position='relative';o.style.cssText=`position: absolute; top: 0; left: 0; width: 100%; min-height: 100%; height: 100%; background: #f3f4f5; z-index: 9999; overflow-y: auto; padding-bottom: 50px;`;pT.parentNode.appendChild(o);}else{o.style.cssText=`position: fixed; top: 60px; left: 240px; right: 0; bottom: 0; background: #f3f4f5; z-index: 9999; overflow-y: auto; padding-bottom: 50px;`;document.body.appendChild(o);}o.style.display='block';});}else{requestAnimationFrame(()=>{o.style.display='block';});}if(!window.
