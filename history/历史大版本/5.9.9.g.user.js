...
(function() {
    ...
    const S={lt:0,wInstUp:0,wInstDn:0,wTotUp:0,wTotDn:0,cls:{}};let isF=!1,pr=new DOMParser(),lCxt=null;const oOp=unsafeWindow.XMLHttpRequest.prototype.open;
    unsafeWindow.XMLHttpRequest.prototype.open=function(){this.
    addEventListener('load',function(){try{if(this.responseType===''||
        ...
        {window.startGegePrecisionEngine();}}}}catch(e){console.warn("[哥哥科技] XHR 拦截器异常:",e.message);}});
                                             oOp.apply(this,arguments);};function s2b(s){if(!s)return 0;let m=s.match(/([\d.]+)\s*(G|M|K)?bps/i);return m?parseFloat(m[1])*(m[2]?.toUpperCase()==='G'?1e9:m[2]?.toUpperCase()==='M'?1e6:m[2]?.toUpperCase()==='K'?1e3:1):0;}function fB(b){return b>=1e6?(b/1e6).toFixed(3)+' Mbps':b>=1e3?(b/1e3).toFixed(2)+' Kbps':Math.round(b)+' bps';}function fBy(b){return(b/=8)>=1048576?(b/1048576).toFixed(3)+' MiB/s':b>=1024?(b/1024).toFixed(2)+' KiB/s':Math.round(b)+' B/s';}function fV(b){return(b/=8)>=1073741824?(b/1073741824).toFixed(3)+' GiB':b>=1048576?(b/1048576).toFixed(2)+' MiB':b>=1024?(b/1024).toFixed(1)+' KiB':Math.round(b)+' B';}function fVD(i,o){let b=i/8,x=o/8;return b>=1073741824?(b/1073741824).toFixed(3)+' | '+(x/1073741824).toFixed(4)+' GiB':b>=1048576?(b/1048576).toFixed(2)+' | '+(x/1048576).toFixed(3)+' MiB':b>=1024?(b/1024).toFixed(2)+' | '+(x/1024).toFixed(1)+' KiB':Math.round(b)+' | '+Math.round(x)+' B'}function fSV(b){return(b/=8)>=1073741824?(b/1073741824).toFixed(3)+'G':b>=1048576?(b/1048576).toFixed(2)+'M':b>=1024?(b/1024).toFixed(1)+'K':Math.round(b)+'B';}function fOT(s){if(s<=0)return"";let d=Math.floor(s/86400),h=Math.floor((s%86400)/3600),m=Math.floor((s%3600)/60),sec=Math.floor(s%60);return d>0?`${d}天${h}时${m}分${sec}秒`:`${h}小时${m}分${sec}秒`;}function nM(m){return m?m.toLowerCase().replace(/-/g,':').replace(/\s/g,''):'';}function pI(n){let o=Object.create(null),c=n.children;for(let i=0;i<c.length;i++){if(c[i].tagName==="ParaName"){let k=c[i].textContent,v="",j=i+1;while(j<c.length&&c[j].tagName!=="ParaName"){if(c[j].tagName==="ParaValue"){v=c[j].textContent;i=j;break;}j++;}o[k]=v;}}return o;}const st=document.createElement('style');
    ...
    appendChild(st);window.gegeRenderedMacs=new Set();async function rSD(pWT=null,sT=null){if(
        ...
        parseFloat(d.UpThroughput)||0)*8000,dT=(parseFloat(d.DownThroughput)||0)*8000;let bN=
                                                                                             d.AliasName||d.HostName||d.DisplayedPictureName||"";cI[m]={upRate:u,dnRate:dn,iface:d.Interface||"",offUp:uT,offDn:dT,onSec:parseInt(d.OnlineDuration||d.OnlineTime||d.LeaseTime||0),name:bN,ip:d.IPAddress||""};cSU+=u;cSD+=dn;}});let ol=document.getElementById('gege-global-overlay'),cM=Object.keys(
...
abU=0,abD=0,
cln={};
for(let k in S.cls){let s=S.cls[k]||{};let cU=Math.max(0,(s.lU||0)-(s.uB||0)-(s.oU||0));let cD=Math.max(0,(s.lD||0)-(s.dB||0)-(s.oD||0));let cC=cI[k]||{};let pN=CONFIG.portMap[cC.iface]||cC.iface||"未知";cln[k]={up:cU,down:cD,status:s.aR?"off":pN,name:cC.name||k,ip:cC.ip||"",raw_up:cC.offUp||0,raw_down:cC.offDn||0};LUp+=s.intUp;LDn+=s.intDn;hpU+=cU;hpD+=cD;}
for(let m in cI){let c=cI[m]||{};abU+=c.offUp||0;abD+=c.offDn||0;tOU+=c.offUp||0;tOD+=c.offDn||0;}
...
bd.innerHTML=`<div class="geek-row"><span class="geek-label">WAN口速率</span><div class="geek-val-box"><span class="c-up geek-fixed-width" id="gb-wan-up-bps"></span><span class="c-down geek-fixed-width" id="gb-wan-down-bps"></span></div><div class="geek-right-box"><span class="c-up" id="gb-wan-up-bytes"></span> | <span class="c-down" id="gb-wan-down-bytes"></span></div></div><div class="geek-row"><span class="geek-label">局域网代数和</span><div class="geek-val-box"><span class="c-up geek-fixed-width" id="gb-lan-up-bps"></span><span class="c-down geek-fixed-width" id="gb-lan-down-bps"></span></div><div class="geek-right-box">实时占比：<span class="c-up" id="gb-perc-up"></span> | <span class="c-down" id="gb-perc-down"></span></div></div><div class="geek-row"><span class="geek-label">LAN：</span><div class="geek-val-box"><span class="c-up geek-fixed-width" id="gb-lan-up-vol"></span><span class="c-down geek-fixed-width" id="gb-lan-down-vol"></span></div><div class="geek-right-box">WAN：<span class="c-up" id="gb-wan-up-vol"></span> | <span class="c-down" id="gb-wan-down-vol"></span></div></div><div class="geek-row"><span class="geek-label">高精流量统计 -></span><div class="geek-val-box"><span class="c-up geek-fixed-width" id="gb-int-up-vol"></span><span class="c-down geek-fixed-width" id="gb-int-down-vol"></span></div><div class="geek-right-box" style="color: #666;">当前总计：<span class="c-up" id="gb-abs-up-vol"></span> | <span class="c-down" id="gb-abs-down-vol"></span></div></div>`;
}
let ol=document.getElementById('gege-global-overlay'),iPO=ol&&ol.style.display==='block',aC=iPO?ol:document;
if(iPO){let ac=document.getElementById('gege-board-anchor');if(ac&&bd.parentNode!==ac)ac.appendChild(bd);}else{let mn=document.querySelector('.el-table')||document.querySelector('.config-item')?.closest('div')||document.querySelector('.main-content');if(mn&&bd.parentNode!==mn.parentNode)mn.parentNode.insertBefore(bd,mn);}
requestAnimationFrame(()=>{
let oDC=Object.create(null);if(!iPO){const M_RX=/([a-fA-F0-9]{2}[:-]){5}[a-fA-F0-9]{2}/;let aI=aC.querySelectorAll('.config-item');for(let n of aI){let mN=n.querySelector('.dev-number'),mM=mN?mN.textContent.match(M_RX):null;if(mM){oDC[mM[0].toLowerCase().replace(/-/g,':')]=n;}}}else{let gI=aC.querySelectorAll('.gege-list-item');for(let n of gI){let m=n.getAttribute('data-gege-mac');if(m)oDC[m]=n;}}
if(bd.parentNode){
...
bd.querySelector('#gb-lan-up-bps').textContent=`🔼 ${fB(sU)}`;
bd.querySelector('#gb-lan-down-bps').textContent=`🔽 ${fB(sD)}`;
...
let it=oDC[m];
if(!it)continue;
const cC=cI[m]||{upRate:0,dnRate:0,iface:"",offUp:0,offDn:0},cS=S.cls[m]||{intUp:0,intDn:0,onS:0};
let tN=it.querySelector('.gege-online-time');
if(tN&&cS.onS>0)tN.textContent=`在线：${fOT(cS.onS)}`;
const dI=it.querySelector('.dev-intro');
if(dI){let bx=dI.querySelector('.gege-up-box');if(!bx){bx=document.createElement('div');bx.className='gege-up-box';bx.innerHTML=`<div class="t-row c-up"><span>↑ <span class="v-vol"></span></span><span class="v-pct"></span></div><div class="zte-thin-bar"><div class="zte-thin-bar-inner up"></div></div>`;dI.appendChild(bx);}let p=tOU>0?((cC.offUp||0)/tOU*100).toFixed(1):0.0;bx.querySelector('.v-vol').textContent=fVD(cS.intUp,cC.offUp);bx.querySelector('.v-pct').textContent=p+'%';bx.querySelector('.zte-thin-bar-inner').style.width=Math.min(p,100)+'%';}
const inf=it.querySelector('.info');
if(inf){Array.from(inf.querySelectorAll('.dev-ip:not(.gege-box *)')).slice(1).forEach(n=>{n.style.display='none';});inf.querySelectorAll('.dev-number:not(.gege-box *)').forEach(n=>{n.style.display='none';});let rB=inf.querySelector('.gege-ratio-box');if(!rB){rB=document.createElement('div');rB.className='gege-ratio-box';rB.innerHTML=`<div class="gege-ratio-top"><span class="v-port"></span><span class="v-interval" style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-weight: normal; font-size: 12.5px; opacity: 0.75; letter-spacing: 0.5px;"><span class="c-up"></span><span style="color:#666; margin:0 3px;">，</span><span class="c-down"></span></span><span class="v-rt-pct"></span></div><div class="gege-ratio-bar"><div class="gege-ratio-bar-inner"></div></div>`;inf.appendChild(rB);}let hqU=cln[m]?cln[m].up:0,hqD=cln[m]?cln[m].down:0,bR=(hqU+hqD)>0?(hqU/(hqU+hqD)*100):0,tC="",tCol="#0059fa";if(CONFIG.calcMode===1){let rt=hqD>0?(hqU/hqD):(hqU>0?Infinity:0);if(rt>CONFIG.ratioExtremeUp){tCol='#ff4c00';tC=(rt===Infinity?'∞':rt.toFixed(2))+'⚠️';}else if(rt>CONFIG.ratioWarnUp){tCol='#ff4c00';tC=(rt*100).toFixed(1)+'%';}else if(rt>=CONFIG.ratioExtremeDown){tCol='#0059fa';tC=(rt*100).toFixed(1)+'%';}else{tCol='#0059fa';let rRt=hqU>0?(hqD/hqU):(hqD>0?Infinity:0);tC=(rRt===Infinity?'∞':rRt.toFixed(1))+'x';}}else{tCol=bR>CONFIG.ratioThreshold?'#ff4c00':'#0059fa';tC=bR.toFixed(1)+'%';}rB.querySelector('.v-port').textContent=CONFIG.portMap[cC.iface]||cC.iface||"未知";rB.querySelector('.v-interval .c-up').textContent=''+fSV(hqU);rB.querySelector('.v-interval .c-down').textContent=''+fSV(hqD);let rtP=rB.querySelector('.v-rt-pct');rtP.textContent=tC;rtP.style.color=tCol;rB.querySelector('.gege-ratio-bar-inner').style.width=Math.min(bR,100)+'%';let dBx=inf.querySelector('.gege-down-box');if(!dBx){dBx=document.createElement('div');dBx.className='gege-down-box';dBx.innerHTML=`<div class="t-row c-down"><span>↓ <span class="v-vol"></span></span><span class="v-pct"></span></div><div class="zte-thin-bar"><div class="zte-thin-bar-inner down"></div></div>`;inf.appendChild(dBx);}let dp=tOD>0?((cC.offDn||0)/tOD*100).toFixed(1):0.0;dBx.querySelector('.v-vol').textContent=fVD(cS.intDn,cC.offDn);dBx.querySelector('.v-pct').textContent=dp+'%';dBx.querySelector('.zte-thin-bar-inner').style.width=Math.min(dp,100)+'%';}
...
if(!enh){enh=document.createElement('div');enh.className='zte-enhance-speed';enh.innerHTML=`<div class="zte-bar-wrap zte-bar-up"><span class="v-val"></span><span class="v-pct"></span></div><div class="zte-bar-wrap zte-bar-down"><span class="v-val"></span><span class="v-pct"></span></div>`;sp.appendChild(enh);}
let pu=sU>0?(cC.upRate/sU*100):0,pd=sD>0?(cC.dnRate/sD*100):0,bU=enh.querySelector('.zte-bar-up'),bD=enh.querySelector('.zte-bar-down');
    bU.style.setProperty('--p-up',Math.min(pu,100)+'%');bU.querySelector('.v-val').textContent=`🔼 ${fBy(cC.upRate)}`;bU.querySelector('.v-pct').textContent=(wU>0?pu.toFixed(1):0.0)+'%';bD.style.setProperty('--p-down',Math.min(pd,100)+'%');bD.querySelector('.v-val').textContent=`🔽 ${fBy(cC.dnRate)}`;bD.querySelector('.v-pct').textContent=(wD>0?pd.toFixed(1):0.0)+'%';}}});}async function bVD(ol,cX){try{let mB=(window.gegeHiddenDevices&&
    Object.keys(window.gegeHiddenDevices).length>0)?'<span style="color: #ff4c00; font-size: 13px; font-weight: normal; margin-left: 10px; font-family: Consolas;">(哥哥科技：智能Mesh适配)</span>':'',h2='',h52='',h58='',hW='';(
        ...
        d.AliasName||d.HostName||'未知设备'),ifc=d.Interface||'',oS=parseInt(d.OnlineDuration||d.OnlineTime||d.LeaseTime||0),tS=fOT(oS),tH=`<div class="gege-online-time" style="color: #999; font-size: 12px; font-family: Consolas; margin-top: 4px;">${tS?'在线：'+tS:''}</div>`,htm=`<div class="col-md-12 col-xs-12 config-item gege-list-item" data-gege-mac="${m}"><div `+
        ...
        `class="dev-name" style="font-weight: bold; color: #333; font-size: 14px;">${nm}</div>${tH}</div></div><div `+
        ...
        htm;}else{hW+=htm;}});requestAnimationFrame(()=>{ol.innerHTML=`<div `+
        ...
        e){requestAnimationFrame(()=>{ol.innerHTML=`<div style="padding: 20px; color: red;">数据渲染失败: ${escapeHTML(e.message)}</div>`;});}}window.createGegeFloatingBtn=function(){if(document.getElementById('gege-floating-btn'))return;let b=
    ...
    lT){aT.classList.add('router-link-exact-active','router-link-active');lT.classList.add('is-active');lT.style.color=
        'rgb(61, 163, 247)';}if(!o){o=document.createElement('div');o.id='gege-global-overlay';let pT=document.querySelector('.page-top');if(pT){pT.parentNode.style.position='relative';o.style.cssText=`position: absolute; top: 0; left: 0; width: 100%; min-height: 100%; height: 100%; background: #f3f4f5; z-index: 9999; overflow-y: auto; padding-bottom: 50px;`;pT.parentNode.appendChild(o);}else{o.style.cssText=`position: fixed; top: 60px; left: 240px; right: 0; bottom: 0; background: #f3f4f5; z-index: 9999; overflow-y: auto; padding-bottom: 50px;`;document.body.appendChild(o);}}o.style.display='block';if(!window.
        ...
        gegeEngineRunning=!0;try{const ts=Date.now(),wR=await fetch(`/?_type=vueData&_tag=vue_home_device_data_no_update_sess&IF_OP=refresh&_=${ts}`);if(
            !wR.ok)throw new Error();const wT=await wR.text(),wST=performance.now();if(!wT.includes('<OBJ_HOME_BASICINFO_ID>'))throw new Error();const wX=
              pr.parseFromString(wT,"text/xml");let bIN=wX.querySelector("OBJ_HOME_BASICINFO_ID Instance"),wI=bIN?pI(bIN):{},cDC=parseInt(
                  wI.AccessDevNum);cDC=isNaN(cDC)?-1:cDC;const lR=await fetch(`/?_type=vueData&_tag=vue_client_data&_=${ts}`);let lT=await lR.text(),lF="";if(lT.includes('<OBJ_CLIENTS_ID>')){let mM=lT.match(/<ParaName>MACAddress<\/ParaName><ParaValue>([^<]+)<\/ParaValue>/g)||[];lF=mM.map(m=>m.replace(/<[^>]+>/g,'')).sort().join('|');}if(cDC!==window.gegeLastDevCount||lF!==window.gegeLastLanFingerprint){const tR=await fetch(
                  `/?_type=vueData&_tag=vue_topo_data&_=${ts}`);if(!tR.ok)throw new Error();let tJ;try{tJ=JSON.parse(await tR.text());}catch(e){throw new Error("Topo Payload corrupted: "+e.message);}let mDC=tJ.agentlay1?.reduce((s,a)=>s+(parseInt(a.accdevCount)||0),0)||0;if(mDC!==window.gegeLastMeshDevCount){window.gegeLastMeshDevCount=mDC;window.
