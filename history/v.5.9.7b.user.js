    ...
    st.innerHTML=`/* `+` */.config-item{`+
        ...
        appendChild(st);window.gegeRenderedMacs=new Set();async function rSD(pWT=null,sT=null){if(
            ...
            S.wInstDn+cWD)/2)*dt;for(let m in cI){if(!S.cls[m]){S.cls[m]={upR:0,dnR:0,intUp:0,intDn:0,uB:
                                                                          CONFIG.readSaveData===1?0:cI[m].offUp,dB:CONFIG.readSaveData===1?0:cI[m].offDn,lU:cI[m].offUp,lD:cI[m].offDn,aR:!1,oU:CONFIG.readSaveData===1?cI[m].offUp:0,oD:CONFIG.readSaveData===1?cI[m].offDn:0};}let cS=S.cls[m],cC=cI[
                                                                                                                                                                                                                                                                                                      m],dU=cC.offUp-cS.lU,dD=cC.offDn-cS.lD;if(dU<0||dD<0){if(dU<0){cS.uB+=dU;cS.dpU=cS.lU;}if(dD<0){cS.dB+=dD;cS.dpD=cS.lD;}cS.aR=!0;}else if(cS.aR){if(dD>2516582400||dU>671088640||(cS.dpD&&dD>=cS.dpD)||(cS.dpU&&dU>=cS.dpU)){cS.uB+=dU;cS.dB+=dD;cS.aR=!1;cS.dpU=0;cS.dpD=0;}}if(cS.lOS!==cC.onSec){cS.onS=cC.onSec;cS.lOS=cC.onSec;}else{cS.onS=(cS.onS||cC.onSec||0)+dt;}cS.lU=cC.offUp;cS.lD=cC.offDn;cS.intUp+=((cS.upR+cC.upRate)/2)*dt;cS.intDn+=((cS.dnR+cC.dnRate)/2)*dt;cS.upR=cC.upRate;cS.dnR=
